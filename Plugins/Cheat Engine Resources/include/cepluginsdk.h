...
#include "celua/lauxlib.h"
#include "celua/lua.h"
#include "celua/lualib.h"
...
typedef struct _PluginVersion {
    unsigned int version;  // write here the minimum version this dll is compatible with (Current supported version: 1
                           // and 2: this SDK only describes 2)
    char* pluginname;  // make this point to a 0-terminated string (allocated memory or static addressin your dll, not
                       // stack)
...
typedef struct _PLUGINTYPE1_INIT {
    ...
    char* shortcut;  // 0 terminated string containing the shortcut in textform. CE will try it's best to parse it to a
                     // valid shortcut
...
typedef struct _ExportedFunctions {
    ...
    CEP_GETMAINWINDOWHANDLE GetMainWindowHandle;  // returns the handle of the main window (for whatever reason, it is
    ...
    PVOID WriteProcessMemory;  // pointer to the pointer of WriteProcessMemory (Change it to hook that api, or use it
                               // yourself)
